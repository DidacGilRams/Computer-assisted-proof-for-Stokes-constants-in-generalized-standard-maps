...
interval E0(IVector b,interval rho)//E0 for psi00+psi01+psi02+psi03+psi04+psi05 (Lemma 3.3)
{
...
interval eta1(interval a2,IVector b,interval rho,interval gamma) // Lemma interval(4.)2
{
	interval r,x;
	r=(rho+interval(3.)*gamma/interval(2.));
	x=interval(2.);//Absolute values of the derivatives of psi0i evaluated at rho
	for(int n=1;n<=7;n++)
	{
...
//Here for the polynomial case kappa=2 and since we took psi00+psi01+psi02+psi03+psi04+psi05 as approximation nu=12
{
...
IVector T0b(IVector b,IVector z)//Rest from T0 for psi00+psi01+psi02+psi03+psi04+psi05+psi06+psi07 at point z. (Lemma 5.1 and using Lemma 3.3)
{
	int Omega=20;
...
IVector T1(interval a2,IVector b,IVector psi,IVector z)//Taylor for psi00+psi01+psi02+psi03+psi04+psi05+psi06+psi07 at point z (Lemma 5.1)
{
...
IVector dpsi10(interval a2,IVector b,interval n,interval rho,interval gamma)//Initial condition for derivative psi1 contain the error given by the theorem of existence, the space is (10+nu_0)
{
	...
	M1=M1+((16+nu0)*M0(a2,b,rho+gamma/2,gamma))/(rho+interval(3.)*gamma/interval(2.));//Here we use that we are computing the derivative at im z= -(rho+2gamma)
...
IVector Wu(interval a2,IVector b,interval re,int L,interval rho,interval gamma)// Psi1 unstable at points re-1-irho and re-irho interated from -L
{
...
IVector dWs(interval a2,IVector b,interval re,int L,interval rho,interval gamma)// psi1, dpsi1 stable at points re-irho and re+1-irho interated from L
{
	interval varrho=-rho-2*gamma;//Height where to compute the difference (where we have defined the interval re(z) in [0,1])
...
void test()//Computer assisted proof for the Stokes constant p_2^{-1}(a_2)
{
	int P=10000,L=200;
	...
	cout << "Lipschidz =" << Lip(a2,b,rho,gamma) << " <=interval(0.5) " << endl;//It has to be less than interval(0.5) to have existence of psi_interval(1.) (see Section interval(3.)3)
	...
	err=exp(interval(2.)*interval::pi()*varrho)*ESS(a2,b,rho,gamma,P);//Error Simpson sum. Here we see which m do we need.
	cout << "ESS " << err << endl;
...
void Lipschidz()//This test can be used to find the minimal rho_0 (Proposition 2.1)
{
	...
	interval rho=interval(8.7),gamma=gamma=interval(0.949327);//rho=9.152 for a_2=6 and rho=8.141 for a_2=0
	IVector b(7);
	b[0]=b1(a2);
	b[1]=b2(a2);
	b[2]=b3(a2);
