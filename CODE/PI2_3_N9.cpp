...
//This code gives an interval for the Stokes constant p_2^{-1}(a_2) for degrees d_{k_0}=2, d_{k_1}=3 and every value of a_2 (coefficient of the second monomial to be introduced in the Stokes()).
//It uses a first approximation for phi_0 with 10 terms, that is, psi_0=psi_0^0+psi_0^1+psi_0^2+psi_0^3+psi_0^4+psi_0^5+psi_0^6+psi_0^7+psi_0^8+psi_0^9
...
//Here for the polynomial case kappa=2./(dk0-1)=2 and nu_N=20+2
{
	interval x;
	x=power(1+4*power(rho,2),interval(0.5));
	x=power((x+1)/(x-1),4);
	x=x*S(rho,gamma,interval(3)+4);
	x=x*(S(rho,gamma,interval(19))+S(rho,gamma,interval(26)));
	return x;
}

//Here we define the coefficients b_{j*d_{k_0}}(d_{k_0}) for j={1,2,3,4,5,6,7,8,9} of the functions psi_0^j
...
interval b8(interval a2)//Coefficient of psi_0^8
{
	interval x;
	x=interval(6.)*(interval(2.)*b1(a2)*b7(a2)+interval(2.)*b2(a2)*b6(a2)+2*b3(a2)*b5(a2)+power(b4(a2),2));
	x=x+a2*(interval(3.)*b7(a2)+interval(3.)*(2*b1(a2)*b6(a2)+2*b2(a2)*b5(a2)+2*b3(a2)*b4(a2))+3*power(b1(a2),2)*b5(a2)+3*b1(a2)*(2*b2(a2)*b4(a2)+power(b3(a2),2))+3*power(b2(a2),2)*b3(a2));
	x=x-interval(2.)*(interval(19.)+binom(19,16)*b1(a2)+binom(19,14)*b2(a2)+binom(19,12)*b3(a2)+binom(19,10)*b4(a2)+binom(19,8)*b5(a2)+binom(19,6)*b6(a2)+binom(19,4)*b7(a2));
	x=x/((interval(18*19))-interval(12.));
	return x;
}

interval b9(interval a2)//Coefficient of psi_0^9
{
	interval x;
	x=interval(6.)*(interval(2.)*b1(a2)*b8(a2)+interval(2.)*b2(a2)*b7(a2)+2*b3(a2)*b6(a2)+2*b4(a2)*b5(a2));
	x=x+a2*(interval(3.)*b8(a2)+interval(3.)*(2*b1(a2)*b7(a2)+2*b2(a2)*b6(a2)+2*b3(a2)*b5(a2)+power(b4(a2),2))+3*power(b1(a2),2)*b6(a2)+3*b1(a2)*(2*b2(a2)*b5(a2)+2*b3(a2)*b4(a2))+3*power(b2(a2),2)*b4(a2)+3*b2(a2)*power(b3(a2),2));
	x=x-interval(2.)*(interval(21.)+binom(21,18)*b1(a2)+binom(21,16)*b2(a2)+binom(21,14)*b3(a2)+binom(21,12)*b4(a2)+binom(21,10)*b5(a2)+binom(21,8)*b6(a2)+binom(21,6)*b7(a2)+binom(21,4)*b8(a2));
	x=x/((interval(20*21))-interval(12.));
	return x;
}
...
{
	interval psi00,psi01,psi02,psi03,psi04,psi05,psi06,psi07,psi08,psi09;
	psi00=interval(1.)/power(rho-1.,2)+1./power(rho+interval(1.),2)-interval(2.)/power(rho,2);
	psi00=psi00*power(rho,2+interval(22.));
	for(int i=1;i<11;i++)
	{
		psi00=psi00-interval(2.)*binom(2+interval(2.)*i-interval(1.),2*i)*power(rho,22-(2*i));
	}
	psi00=abs(psi00);
	...
	psi01=psi01*power(rho,2+interval(22.));
	for(int i=1;i<10;i++)
	{
		psi01=psi01-interval(2.)*binom(2+interval(2.)*(i+1)-interval(1.),2*i)*power(rho,22-2*(i+1));
	}
	...
	psi02=psi02*power(rho,2+interval(22.));
	for(int i=1;i<9;i++)
	{
		psi02=psi02-interval(2.)*binom(2+interval(2.)*(i+2)-interval(1.),2*i)*power(rho,22-2*(i+2));
	}
	...
	psi03=psi03*power(rho,2+interval(22.));
	for(int i=1;i<8;i++)
	{
		psi03=psi03-interval(2.)*binom(2+interval(2.)*(i+3)-interval(1.),2*i)*power(rho,22-2*(i+3));
	}
	...
	psi04=psi04*power(rho,2+interval(22.));
	for(int i=1;i<7;i++)
	{
		psi04=psi04-interval(2.)*binom(2+interval(2.)*(i+4)-interval(1.),2*i)*power(rho,22-2*(i+4));
	}
	...
	psi05=psi05*power(rho,2+interval(22.));
	for(int i=1;i<6;i++)
	{
		psi05=psi05-interval(2.)*binom(2+interval(2.)*(i+5)-interval(1.),2*i)*power(rho,22-2*(i+5));
	}
	...
	psi06=psi06*power(rho,2+interval(22.));
	for(int i=1;i<5;i++)
	{
		psi06=psi06-interval(2.)*binom(2+interval(2.)*(i+6)-interval(1.),2*i)*power(rho,22-2*(i+6));
	}
	...
	psi07=psi07*power(rho,2+interval(22.));
	for(int i=1;i<4;i++)
	{
		psi07=psi07-interval(2.)*binom(2+interval(2.)*(i+7)-interval(1.),2*i)*power(rho,22-2*(i+7));
	}
	psi07=abs(b[6])*abs(psi07);
	psi08=interval(1.)/power(rho-1,2+interval(16.))+interval(1.)/power(rho+1,2+interval(16.))-interval(2.)/power(rho,2+interval(16.));
	psi08=psi08*power(rho,2+interval(22.));
	for(int i=1;i<3;i++)
	{
		psi08=psi08-interval(2.)*binom(2+interval(2.)*(i+8)-interval(1.),2*i)*power(rho,22-2*(i+8));
	}
	psi08=abs(b[7])*abs(psi08);
	psi09=interval(1.)/power(rho-1,2+interval(18.))+interval(1.)/power(rho+1,2+interval(18.))-interval(2.)/power(rho,2+interval(18.));
	psi09=psi09*power(rho,2+interval(22.));
	for(int i=1;i<3;i++)
	{
		psi09=psi09-interval(2.)*binom(2+interval(2.)*(i+9)-interval(1.),2*i)*power(rho,22-2*(i+9));
	}
	psi09=abs(b[8])*abs(psi09);
	return psi00+psi01+psi02+psi03+psi04+psi05+psi06+psi07+psi08+psi09;
}
...
{
	interval x,y,psi00,psi01,psi02,psi03,psi04,psi05,psi06,psi07,psi08,psi09;
	...
	psi08=abs(b[7])*power(rho,-2-interval(16.));
	psi09=abs(b[8])*power(rho,-2-interval(18.));
	x=interval(2.)*(psi01*psi09)+interval(2.)*(psi02*(psi08+psi09))+2*psi03*(psi07+psi08+psi09)+2*psi04*(psi06+psi07+psi08+psi09)+power(psi05+psi06+psi07+psi08+psi09,2);//Function f(psi1) evaluated at 0
	x=interval(6.)*x;
	y=interval(3.)*power(psi00,2)*psi09+interval(3.)*psi00*(interval(2.)*psi01*(psi08+psi09)+interval(2.)*psi02*(psi07+psi08+psi09)+2*psi03*(psi06+psi07+psi08+psi09)+2*psi04*(psi05+psi06+psi07+psi08+psi09)+power(psi05+psi06+psi07+psi08+psi09,2));
	y=y+interval(3.)*power(psi01,2)*(psi07+psi08+psi09)+interval(3.)*psi01*(2*psi02*(psi06+psi07+psi08+psi09)+2*psi03*(psi05+psi06+psi07+psi08+psi09)+power(psi04+psi05+psi06+psi07+psi08+psi09,2));
	y=y+3*power(psi02,2)*(psi05+psi06+psi07+psi08+psi09)+3*psi02*(psi03*(psi04+psi05+psi06+psi07+psi08+psi09)+power(psi04+psi05+psi06+psi07+psi08+psi09,2))+power(psi03+psi04+psi05+psi06+psi07+psi08+psi09,3);
	x=x+a2*y;
	return x*power(rho,2+interval(22.));
}
...
interval R(interval a2,IVector b,interval rho,interval M0)//Bounding higher order terms R^b (Lemma 5.2.11)
{
	...
	psi=CN(b,0,rho);
	c=interval(12.)*M0/power(rho,interval(20.));
	c=c+interval(3.)*a2*(power(interval(1.)+psi+M0/power(rho,interval(20.)),2)-power(interval(1.)+psi,2));
	return abs(c);
}
...
interval eta1(interval a2,IVector b,interval rho,interval gamma)//Computation of eta_1^b (Lemma 5.3.3)
{
	...
	x=x+(2*M0(a2,b,rho,gamma))/(c(gamma)*power(rho,19));
	return x+(3*M0(a2,b,rho+gamma/2,gamma))/(power(r,20));
}
...
//Here for the polynomial case kappa=2 and nuN=22
{
	return eta1(a2,b,rho,gamma)*eta2(a2,b,rho,gamma)*(Sd(rho,gamma,interval(20))+Sd(rho,gamma,interval(27)));
}
...
interval Ad(interval a2,IVector b,interval rho, interval gamma)//Computation of A^{d,b} (Lemma 5.3.1)
{
	...
	c=interval(2.)*M*(interval(12.)+((interval(6.)*a2)/power(r,interval(2.)))*(1+CN(b,0,r)+(M/power(r,20))));
	return c;
}

interval Lipd(interval a2,IVector b,interval rho,interval gamma)//Bound for the operator G^d[A^d*] (Lemma 5.3.8)
{
	return Gd(a2,b,rho,gamma)*Ad(a2,b,rho,gamma)*power(rho+interval(3.)*gamma/interval(2.),-20);
}
...
IVector psi0(IVector b,IVector z)//Computation of psi_0=psi_0^0+psi_0^1+psi_0^2+psi_0^3+psi_0^4+psi_0^5+psi_0^6+psi_0^7+psi_0^8+psi_0^9
{
	IVector x(2);
...
IVector dpsi0(IVector b,IVector z)//Derivative of psi_0=psi_0^0+psi_0^1+psi_0^2+psi_0^3+psi_0^4+psi_0^5+psi_0^6+psi_0^7+psi_0^8+psi_0^9
{
	IVector x(2);
	x=-2*power(z,-1-2);
	for(int n=1;n<=9;n++)
...
IVector T0(IVector b,IVector z)//T_0^Omega(z) for psi00+psi01+psi02+psi03+psi04+psi05+psi06+psi07+psi08+psi09 (Lemma 5.4.4)
{
	int Omega=25;
	interval nu0=interval(2.);
	IVector psi00(2),psi01(2),psi02(2),psi03(2),psi04(2),psi05(2),psi06(2),psi07(2),psi08(2),psi09(2);
	psi00[0]=interval(0.);
	psi00[1]=psi00[0];
	psi01=psi00;
	psi02=psi00;
...
IVector T0b(IVector b,IVector z)///Sum of T_0^{n,b}(z) (Lemma 5.4.4 and Remark 5.4.5)
{
	...
	psi08=abs(b[7])*binom(2*8+2*pNn(Omega,8)-1+nu0,2*pNn(Omega,8))*(power(c-1,-(2*8+2*pNn(Omega,8)+nu0))+power(c+1,-(2*8+2*pNn(Omega,8)+nu0))-2*power(c,-(2*8+2*pNn(Omega,8)+nu0)));
	psi09=abs(b[8])*binom(2*9+2*pNn(Omega,9)-1+nu0,2*pNn(Omega,9))*(power(c-1,-(2*9+2*pNn(Omega,9)+nu0))+power(c+1,-(2*9+2*pNn(Omega,9)+nu0))-2*power(c,-(2*9+2*pNn(Omega,9)+nu0)));
	x[0]=interval(-1.,1.)*(psi00+psi01+psi02+psi03+psi04+psi05+psi06+psi07+psi08+psi09);
	x[1]=x[0];
	return x;
}

IVector T1(interval a2,IVector b,IVector psi,IVector z)//T_1^Omega for psi00+psi01+psi02+psi03+psi04+psi05+psi06+psi07+psi08+psi09 at point z (Lemma 5.4.4)
{
	IVector x(2),y(2),psi00(2),psi01(2),psi02(2),psi03(2),psi04(2),psi05(2),psi06(2),psi07(2),psi08(2),psi09(2);
	psi00=power(z,-2);
	...
	psi08=b[7]*power(z,-2-interval(16.));
	psi09=b[8]*power(z,-2-interval(18.));
	x=interval(2.)*cprod(psi00,psi)+interval(2.)*cprod(psi01,psi09+psi)+interval(2.)*cprod(psi02,psi08+psi09+psi)+2*cprod(psi03,psi07+psi08+psi09+psi)+2*cprod(psi04,psi06+psi07+psi08+psi09+psi)+power(psi05+psi06+psi07+psi08+psi09+psi,2);
	x=interval(6.)*x;
	y=interval(3.)*cprod(power(psi00,2),psi09+psi)+interval(3.)*cprod(psi00,interval(2.)*cprod(psi01,psi08+psi09+psi)+2*cprod(psi02,psi07+psi08+psi09+psi)+2*cprod(psi03,psi06+psi07+psi08+psi09+psi)+2*cprod(psi04,psi05+psi06+psi07+psi08+psi09+psi)+power(psi05+psi06+psi07+psi08+psi09+psi,2));
	y=y+interval(3.)*cprod(power(psi01,2),psi06+psi07+psi08+psi)+interval(3.)*cprod(psi01,2*cprod(psi02,psi05+psi06+psi07+psi08+psi)+2*cprod(psi03,psi04+psi05+psi06+psi07+psi08+psi)+power(psi04+psi05+psi06+psi07+psi08+psi09+psi,2));
	y=y+3*cprod(power(psi02,2),psi05+psi06+psi07+psi08+psi09+psi)+3*cprod(psi02,2*cprod(psi03,psi04+psi05+psi06+psi07+psi08+psi09+psi)+power(psi04+psi05+psi06+psi07+psi08+psi09+psi,2))+power(psi03+psi04+psi05+psi06+psi07+psi08+psi09+psi,3);
	x=x+a2*y;
	return x;
}
...
IVector psi10(interval a2,IVector b,interval n,interval rho,interval gamma)//Initial condition for psi_1^{u,s} (using Proposition 5.1.2)
{
	...
	psi10[0]=interval(-1.,1.)*M0(a2,b,rho,gamma)/(power(mod(z),22));
	psi10[1]=psi10[0];
	return psi10;
}
...
IVector dpsi10(interval a2,IVector b,interval n,interval rho,interval gamma)//Initial condition for the derivative of psi_1^s (see Remark 5.3.4)
{
	...
	M1=M1+((20+nu0)*M0(a2,b,rho+gamma/2,gamma))/(rho+interval(3.)*gamma/interval(2.));//Here we use that we are computing the derivative at im z= -i(rho+2gamma)
	M1=M1/(power(mod(z),20+nu0));
...
IVector SSum(interval a2,IVector b,int L,interval rho,interval gamma,int P)// Simpson sum of p2(z)*e^(2pi*z) between -ivarrho and 1-ivarrho 
{
	...
	for(int i=0;i<P+1;i++)
	{
		...
		z[1]=-varrho;
		z[0]=interval(s-1);
		err=power(mod(z),-21-nu0)*zeta11(a2,b,rho,gamma)*interval(-1.,1.);//Observe that this error decreses when considering a bigger rho, which means smaller interval in the end
		y=cprod(deltaR,dpsisL+err);
		z[0]=interval(s);
		err=power(mod(z),-21-nu0)*zeta11(a2,b,rho,gamma)*interval(-1.,1.);
...
interval ESS(interval a2,IVector b,interval rho,interval gamma,int P)//Error for the Simpson sum (Lemma 5.1.6)
{
	...
	x=x+(zeta11(a2,b,rho,gamma)/power(r,21+nu0));
	x=(x*M0(a2,b,rho+gamma/2,gamma))/power(r,20+nu0);
...
{
	int P=20000,L=250;
	interval rho=interval(9.192),gamma=interval(0.949327);
...
{
	interval a2=interval(0);
	interval rho=interval(9.2),gamma=gamma=interval(0.949327);//10.25 for 6.1, 10.28 for 6.3, 9.192 for 0
	IVector b(9);
	b[0]=b1(a2);
	b[1]=b2(a2);
	b[2]=b3(a2);
